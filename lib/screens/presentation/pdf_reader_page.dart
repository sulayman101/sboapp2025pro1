  ...
  _pdfViewWidget() {
    return SfPdfViewer.file(
      ...
      onPageChanged: (PdfPageChangedDetails details) async {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("${_pdfViewerController.pageNumber} Guest users can view only up to 5 pages. Please sign in for full access.")));

        if (_pdfViewerController.pageNumber > 5) {
          _pdfViewerController.jumpToPage(5); // Restrict to page 5
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text("Guest users can view only up to 5 pages. Please sign in for full access.")),
          );
          setState(() {
        ...
        // Save last read page
        final prefs = await SharedPreferences.getInstance();
        prefs.setInt('lastPage', currentPage);
        prefs.setString('lastBookId', widget.bookModel!.bookId.toString());
        setState(() {
      ...
      onDocumentLoadFailed: (failed) {
        customizedSnackBar(
          ...
          message: "Can't open file. Please try again.",
          contentType: ContentType.failure,
        );
      },
      onHyperlinkClicked: (PdfHyperlinkClickedDetails details) => _hyperLinks(),
   ...
   _pdfViewWidget(){
    return SfPdfViewer.file(
      ...
      onDocumentLoaded: (loaded)async {
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("You reach limit of Guest User")));
      ...
      onDocumentLoadFailed: (failed) {
        customizedSnackBar(
            title: "Failed request",
            message: "can't opened please try again",
            contentType: ContentType.failure);
      },
      onHyperlinkClicked: (PdfHyperlinkClickedDetails details)=> _hyperLinks(),
      enableHyperlinkNavigation: true,
      ...
      onPageChanged: (PdfPageChangedDetails details) async {
        final prefs = await SharedPreferences.getInstance();
        prefs.setInt('lastPage', details.newPageNumber);
        prefs.setString('lastBookId', widget.bookModel!.bookId.toString()); // You must set this from your book info
        final isGuest = Provider.of<AuthServices>(context, listen: false).isGuest;
        if(isGuest){
          _pdfViewerController.jumpToPage(5); // Restrict to page 5
        }
              ...
              children: [
                ...
                Padding(
                  ...
                  child: widget.bookModel != null
                      ...
                      : const Text("You can not immediately"),
                ),
                SizedBox(
