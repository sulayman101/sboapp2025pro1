...
class _HomePageState extends State<HomePage> with TickerProviderStateMixin {
  ...
  StreamSubscription<DatabaseEvent>? _waitingListener;
  StreamSubscription<DatabaseEvent>? _updatesListener;

  int _selected = 1;

  // App version and update variables
  String? appVersion;
  bool waiting = false;
  ...
  checkDot() async {
    ...
    try {
      ...
      if (isRead != null && mounted && !_isDisposed) {
        setState(() => newNotify = !isRead);
      }
    } catch (e) {
      log('Error checking dot: $e');
    }
  }
  ...
  void _initDynamicLinks() async {
    ...
    try {
      ...
      if (initialLink != null && mounted && !_isDisposed) {
      ...
      _linkSubscription = _appLinks.uriLinkStream.listen((Uri uri) {
        ...
        log('Error handling app link: $err');
      });
    } catch (e) {
      log('Error initializing dynamic links: $e');
    }
  }
  ...
  void _processIncomingLink(Uri uri) {
    ...
    try {
      ...
      if (pathSegments.length >= 2) {
        ...
        String bookId = pathSegments[1];

        log('Navigating to category: $category, bookId: $bookId');

        // Navigate to your BookShareInfoView
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => BookShareInfoView(
              category: category,
              bookId: bookId,
            ),
          ),
      ...
      log('Error processing incoming link: $e');
    }
  }
  ...
  Future<void> _initialize() async {
    ...
    try {
      ...
      if (_available && !_isDisposed) {
        _fetchSubscriptions();
      }
    } catch (e) {
      log('Error initializing in-app purchase: $e');
    }
  }
  ...
  void _listenToPurchaseUpdated(List<PurchaseDetails> purchaseDetailsList) {
    ...
    try {
      ...
      log('Error processing purchase update: $e');
    }
  }
  ...
  void _showPurchaseOptionsDialog(providerLocale) {
    if (_products == null || _products!.isEmpty || _isDisposed) return;
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                customText(
                    text: providerLocale.bodyPurchaseNote,
                    fontSize: 10,
                    maxLines: 10,
  ...
  void getDeviceAndVersion() async {
    ...
    try {
      ...
      log('Error getting device version: $e');
    }
  }
  ...
  Future<void> checkForUpdate() async {
    ...
    try {
      ...
      // Handle known error for development/testing
      if (e.toString().contains('ERROR_APP_NOT_OWNED')) {
        log('App not from Play Store - skipping update check');
      } else {
  ...
  void updateApp() async {
    ...
    try {
      ...
      InAppUpdate.completeFlexibleUpdate().then((value) {}).catchError((e) {
        log('Error completing flexible update: $e');
      });
    } catch (e) {
  ...
  void checkUser() async {
    ...
    try {
      ...
      if (await isBanned && mounted && !_isDisposed) {
  ...
  Future shareApp(providerLocale) async {
    ...
    try {
      String appLink =
          'https://play.google.com/store/apps/details?id=com.dsd.sboapp';
      await Share.share('${providerLocale.bodyShareAppLink} $appLink');
    } catch (e) {
  ...
  void checkUserDevice(String userDevice) async {
    ...
    try {
      ...
      log('Error checking user device: $e');
    }
  }
  ...
  void allowNotification() async {
    ...
    try {
      ...
      log('Error allowing notification: $e');
    }
  }
  ...
  void initActions() {
    ...
    // Initialize purchase stream subscription with proper error handling
  ...
  void checkUpdating() {
    ...
    try {
      ...
      _waitingListener = databaseReference
        ...
        if (!_isDisposed && mounted) {
          ...
          if (waiting) {
            Navigator.pushReplacement(context,
                MaterialPageRoute(builder: (context) => const UpdatingApp()));
          }
        }
      }, onError: (error) {
        log('Error listening to waiting updates: $error');
      });
    } catch (e) {
      log('Error setting up update listener: $e');
    }
  }
  ...
  Future<void> getCheckUpdating() async {
    ...
    try {
      ...
      // Listen for updates
      final snapshot = await databaseReference.child('$dbName/updates').get();
      if (snapshot.exists && !_isDisposed) {
        ...
        if (updating && mounted && !_isDisposed) {
          if (int.parse(currentVersion!.replaceAll('.', '').toString()) <
              int.parse(newVersion.replaceAll('.', '').toString())) {
            checkUpdating();
          }
        }
      }
  ...
  @override
  void dispose() {
    // Set disposal flag first to prevent any new operations
    _isDisposed = true;

    // Cancel all stream subscriptions
    _linkSubscription?.cancel();
    _subscription?.cancel();
    _waitingListener?.cancel();
    _updatesListener?.cancel();

    // Clear references to prevent memory leaks
    _linkSubscription = null;
    _subscription = null;
    _waitingListener = null;
    _updatesListener = null;
  ...
  Widget build(BuildContext context) {
    if (_isDisposed) {
      return const SizedBox(); // Return empty widget if disposed
    }
                        ...
                        return Padding(
                          ...
                          child: Material(
                            ...
                            child: ListTile(
                                ...
                                subtitle: const Text("Sign up or Sign In"),
...
//     }
//
//     // Listen to incoming links (replaces uriLinkStream.listen)
//     _linkSubscription = _appLinks.uriLinkStream.listen((Uri uri) {
//       _processIncomingLink(uri);
//     }, onError: (err) {
//       log('Error handling app link: $err');
...
//       String bookId = pathSegments[1]; // e.g., "9187d2c0-c824-1089-a3f1-a3bd607cedf7"
//
//       log('Navigating to category: $category, bookId: $bookId');
//
//       // Navigate to your BookShareInfoView
...
//   checkUpdating() {
//     final DatabaseReference databaseReference = FirebaseDatabase.instance.ref();
//     databaseReference.child('$dbName/updates/waiting').onValue.listen((onData) {
//       setState(() => waiting = bool.parse(onData.snapshot.value.toString()));
//     });
//     if (waiting) {
//       Navigator.pushReplacement(context,
...
//                           child: Material(
//                               borderRadius: BorderRadius.circular(15),
//                             child: ListTile(title: Text("Geust user"), subtitle: Text("Sign up or Sing In"), trailing: Icon(Icons.arrow_forward_ios), onTap:(){
//                               Navigator.push(
//                                 context,
